<!DOCTYPE html>
<html lang="pt-BR">
<?php
session_start();

if(isset($_SESSION['email']) && isset($_SESSION['nome'])) {
  $email_usuario = $_SESSION['email'];
  $nome_usuario = $_SESSION['nome'];
} else {
  // Caso não esteja logado, redirecione para a página de login
  header('Location: login.html');
  exit(); // Encerra o script após redirecionar
}
?>
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="shortcut icon" href="../imgs/favicon.png" type="image/x-icon" />
    <link rel="stylesheet" href="../styles/component-navegacao/component-navegacao.css">
    <link rel="stylesheet" href="../styles/perfil/perfil.css" />
    <link rel="stylesheet" href="../styles/perfil/perfil-responsive.css"/>
    <link rel="stylesheet" href="../styles/global.css" />
    <title>Perfil</title>
  </head>
  <body>
  <header>
    <img src="../imgs/Krow-index-logo.png" alt="Krow logo" />
    <div class="perfil-usuario">
      <img src="../imgs/icon-user.png" alt="" />
      <p><?php echo $email_usuario ?></p>
    </div>
  </header>

    <div id="content-home-contratado">
      <!-- barra lateral de ICONES -->
      <nav class="activity-bar">
        <div class="activity-icons">
          <a id="category-icon" href="javascript:void(0);"><img src="../imgs/svg/icon-category.svg" alt="Icone de categorias"></a>
          <a href="../pages/home-contratado.php"><img src="../imgs/svg/work-bag.svg" alt="Icone de Prestador de serviço"></a>
          <a href="../pages/home-contratante.php"><img src="../imgs/svg/work-contract.svg" alt="Icone de Contratante de serviço"></a>
          <a href="perfil.html"><img src="../imgs/svg/icon-profile.svg" alt="Icone de perfil"></a>
        </div>

        <form method="POST" action="../src/backend/loginpaginaanterior.php">
        <input type="hidden" name="redirect" value="<?php echo urlencode($_SERVER['REQUEST_URI']); ?>">
        <button class="Btn" type="submit" name="sair">
          <div class="sign">
            <svg viewBox="0 0 512 512">
              <path d="M377.9 105.9L500.7 228.7c7.2 7.2 11.3 17.1 11.3 27.3s-4.1 20.1-11.3 27.3L377.9 406.1c-6.4 6.4-15 9.9-24 9.9c-18.7 0-33.9-15.2-33.9-33.9l0-62.1-128 0c-17.7 0-32-14.3-32-32l0-64c0-17.7 14.3-32 32-32l128 0 0-62.1c0-18.7 15.2-33.9 33.9-33.9c9 0 17.6 3.6 24 9.9zM160 96L96 96c-17.7 0-32 14.3-32 32l0 256c0 17.7 14.3 32 32 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-64 0c-53 0-96-43-96-96L0 128C0 75 43 32 96 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32z"></path>
            </svg>
          </div>
          <div class="text">Desconectar</div>
        </button>
      </form>
      </nav>
      
      <!-- barra lateral de CATEGORIAS -->
      <nav id="category-menu" class="category-menu">
        <button id="back-button" class="back-button"><img src="../imgs/svg/back.svg" alt="Voltar"></button>
        <h3>Categorias</h3>
        <ul>
          <li><a href="#">Todos</a></li>
          <li><a href="#">Musical</a></li>
          <li><a href="#">Desenho</a></li>
          <li><a href="#">Edição de Vídeo</a></li>
          <li><a href="#">Edição de Foto</a></li>
          <li><a href="#">Dublagem</a></li>
          <li><a href="#">Cozinheiro</a></li>
          <li><a href="#">Garçom</a></li>
          <li><a href="#">Barista</a></li>
          <li><a href="#">Doméstico</a></li>
          <li><a href="#">Babá</a></li>
          <li><a href="#">Motorista</a></li>
          <li><a href="#">Entregador</a></li>
          <li><a href="#">Acompanhante</a></li>
          <li><a href="#">Atuação</a></li>
          <li><a href="#">Didático</a></li>
          <li><a href="#">Outros</a></li>
        </ul>
      </nav>

      <main>
        <!-- ============= LADO ESQUERDO ============= -->
        <div class="esquerdo">
      <div class="textos">
        <label>Olá eu sou</label>
        <h1><?php echo $nome_usuario; ?></h1>
      </div>

          <div class="card">
            <a class="socialContainer containerOne" href="#">
              <svg viewBox="0 0 16 16" class="socialSvg instagramSvg"> <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z"></path> </svg>
            </a>

            <a class="socialContainer containerTwo" href="#">
              <svg viewBox="0 0 448 512" class="socialSvg linkdinSvg"><path d="M100.28 448H7.4V148.9h92.88zM53.79 108.1C24.09 108.1 0 83.5 0 53.8a53.79 53.79 0 0 1 107.58 0c0 29.7-24.1 54.3-53.79 54.3zM447.9 448h-92.68V302.4c0-34.7-.7-79.2-48.29-79.2-48.29 0-55.69 37.7-55.69 76.7V448h-92.78V148.9h89.08v40.8h1.3c12.4-23.5 42.69-48.3 87.88-48.3 94 0 111.28 61.9 111.28 142.3V448z"></path></svg>
            </a>
                
            <a class="socialContainer containerThree" href="#">
              <svg viewBox="0 0 16 16" class="socialSvg whatsappSvg"> <path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z"></path> </svg>
            </a>
          </div>             
              
          <div class="experiencia"> 

            <div class="xp">
              <label id="text1"> 1+</label>
              <p id="text2">Descrição</p>
            </div>
            <div class="xp">
              <label id="text1"> 1+</label>
              <p id="text2">Descrição</p>
            </div>
            <div class="xp">
              <label id="text1"> 1+</label>
              <p id="text2">Descrição</p>
            </div>
            <div class="xp">
              <label id="text1"> 1+</label>
              <p id="text2">Descrição</p>
            </div>
            
            </div>
    
          </div>

        <!-- ============= LADO DIREITO ============= -->
        <div class="direito">
          <img src="https://avatars.githubusercontent.com/u/104092305?v=4" alt="">
          <button class="Btn-edit" id="editarBtn">
            Editar
            <svg viewBox="0 0 512 512" class="svg">
              <path d="M410.3 231l11.3-11.3-33.9-33.9-62.1-62.1L291.7 89.8l-11.3 11.3-22.6 22.6L58.6 322.9c-10.4 10.4-18 23.3-22.2 37.4L1 480.7c-2.5 8.4-.2 17.5 6.1 23.7s15.3 8.5 23.7 6.1l120.3-35.4c14.1-4.2 27-11.8 37.4-22.2L387.7 253.7 410.3 231zM160 399.4l-9.1 22.7c-4 3.1-8.5 5.4-13.3 6.9L59.4 452l23-78.1c1.4-4.9 3.8-9.4 6.9-13.3l22.7-9.1v32c0 8.8 7.2 16 16 16h32zM362.7 18.7L348.3 33.2 325.7 55.8 314.3 67.1l33.9 33.9 62.1 62.1 33.9 33.9 11.3-11.3 22.6-22.6 14.5-14.5c25-25 25-65.5 0-90.5L453.3 18.7c-25-25-65.5-25-90.5 0zm-47.4 168l-144 144c-6.2 6.2-16.4 6.2-22.6 0s-6.2-16.4 0-22.6l144-144c6.2-6.2 16.4-6.2 22.6 0s6.2 16.4 0 22.6z"></path>
            </svg>
          </button>
        </div>

      </main>

      <!-- Conteúdo do Formulário de Edição -->
      <div id="myModal" class="modal">
        <div class="modal-content">
          <span class="close">&times;</span>
          <form id="formularioEdicao">
              
            <div class="form">
                <input class="input" placeholder="Nome" required="" type="text">
                <span class="input-border"></span>
            </div>
            <div class="form">
                <input class="input" placeholder="Idade" required="" type="text">
                <span class="input-border"></span>
            </div>
            <div class="form">
                <input class="input" placeholder="Contato" required="" type="text">
                <span class="input-border"></span>
            </div>
            <div class="form">
                <input class="input" placeholder="Instagram" required="" type="text">
                <span class="input-border"></span>
            </div>
            <div class="form">
                <input class="input" placeholder="Linkedin" required="" type="text">
                <span class="input-border"></span>
            </div>
            <div class="form">
                <input class="input" placeholder="Genero" required="" type="text">
                <span class="input-border"></span>
            </div>
            <div class="form">
                <input class="input" placeholder="Experiencias" required="" type="text">
                <span class="input-border"></span>
            </div>
            <input class="login-button" type="submit" value="SALVAR" />

          </form>
        </div>
      
      </div>
    <script>
      // Scroll para caixa de esperiencias
      const experiencia = document.querySelector('.experiencia');
      let isDown = false;
      let startX;
      let scrollLeft;
  
      experiencia.addEventListener('mousedown', (e) => {
          isDown = true;
          startX = e.pageX - experiencia.offsetLeft;
          scrollLeft = experiencia.scrollLeft;
          experiencia.classList.add('active');
          e.preventDefault(); // Evita seleção de texto
      });
  
      experiencia.addEventListener('mouseleave', () => {
          isDown = false;
          experiencia.classList.remove('active');
      });
  
      experiencia.addEventListener('mouseup', () => {
          isDown = false;
          experiencia.classList.remove('active');
      });
  
      experiencia.addEventListener('mousemove', (e) => {
          if (!isDown) return;
          e.preventDefault();
          const x = e.pageX - experiencia.offsetLeft;
          const walk = (x - startX) * 2; // Ajuste a velocidade do scroll aqui
          experiencia.scrollLeft = scrollLeft - walk;
      });
  
      document.addEventListener('mouseup', () => {
          isDown = false;
          experiencia.classList.remove('active');
      });

      // =======================================================

      // edição de formulario
      document.getElementById('editarBtn').addEventListener('click', function() {
      var modal = document.getElementById('myModal');
      modal.classList.add('show');
      });

    // JavaScript para fechar o modal
    document.querySelector('.close').addEventListener('click', function() {
        var modal = document.getElementById('myModal');
        modal.classList.remove('show');
    });

    // Fecha o modal se o usuário clicar fora do conteúdo
    window.addEventListener('click', function(event) {
        var modal = document.getElementById('myModal');
        if (event.target == modal) {
            modal.classList.remove('show');
        }
    });
    </script>
    <script src="../src/frontend/category-bar.js"></script>
  </body>
</html>